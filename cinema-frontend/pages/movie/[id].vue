  ...
  <div class="max-w-5xl mx-auto px-4 py-6">
    ...
    <div v-else-if="movie" class="bg-white rounded-lg shadow-lg overflow-hidden">
      <div class="md:flex">
        ...
        <div class="p-6 md:w-2/3">
          ...
          <button @click="showBookingModal = true" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition-colors">
            Reservar Entrades
          </button>
        </div>
      </div>
    ...
    <div class="bg-white rounded-lg p-4 max-w-md w-full">
      ...
      <form @submit.prevent="submitBooking" class="space-y-3">
        <div>
          ...
          <input type="text" v-model="booking.name" required class="mt-1 block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200">
        </div>
        <div>
          <label class="block text-xs font-medium text-gray-700">Correu electrònic</label>
          <input type="email" v-model="booking.email" required class="mt-1 block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200">
        </div>
        <div class="flex justify-end space-x-2">
          <button type="button" @click="showBookingModal = false" class="px-3 py-1 text-xs border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
            Cancel·lar
          </button>
    ...
    <div v-if="showSuccessModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full text-center">
        ...
        <p class="text-gray-600 mb-4 text-sm">La teva reserva s'ha realitzat correctament. Rebràs un correu de confirmació.</p>
        ...
        <button @click="showSuccessModal = false" class="px-4 py-2 text-sm bg-red-600 text-white rounded-md hover:bg-red-700">
          Tancar
        </button>
      </div>
    </div>
