  ...
  <div class="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 bg-white rounded-lg shadow p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Classificació</label>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Horari</label>
          <select v-model="filters.showTime"
            class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Ordena per</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <div v-for="movie in movies" :key="movie.id" class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="relative">
          <img :src="movie.poster_url" :alt="movie.title" class="w-full h-96 object-cover" />
          <div class="absolute top-4 right-4 bg-red-600 text-white px-2 py-1 rounded">
            {{ movie.rating }}
          </div>
        </div>
        <div class="p-6">
    ...
    <div v-if="showBookingModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg max-w-md w-full p-6">
        <h3 class="text-2xl font-bold mb-4">Reserva entrades per a {{ selectedMovie?.title }}</h3>
        <form @submit.prevent="submitBooking" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700">Horari</label>
            <select v-model="booking.showTime" required
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
              <option v-for="time in selectedMovie?.showTimes" :key="time" :value="time">
                {{ time }}
              </option>
            </select>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700">Nombre d'entrades</label>
            <input type="number" v-model="booking.tickets" min="1" required
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" />
          </div>
    ...
    <div v-if="showSuccessModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg max-w-md w-full p-6 text-center">
        ...
        <p class="text-gray-600 mb-4">
          Gràcies per reservar entrades per a {{ selectedMovie?.title }}. Hem enviat la confirmació al teu correu
          electrònic.
        </p>
